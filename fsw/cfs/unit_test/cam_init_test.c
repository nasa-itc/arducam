...
static void CAM_Init_Test_RegisterError(void) {
  Ut_CFE_ES_SetReturnCode(UT_CFE_ES_REGISTERAPP_INDEX, CFE_ES_ERR_APP_REGISTER,
                          1);
  cam_Main();
  UtAssert_True(CAM_AppData.RunStatus == CFE_ES_RunStatus_APP_ERROR,
                "cam run status");
}
...
static void CAM_Init_Test_ChildTaskError(void) {
  Ut_CFE_ES_SetReturnCode(UT_CFE_ES_CREATECHILDTASK_INDEX,
                          CFE_ES_ERR_CHILD_TASK_CREATE, 1);
  cam_Main();
  UtAssert_True(CAM_AppData.RunStatus == CFE_ES_RunStatus_APP_ERROR,
                "cam run status");
}
