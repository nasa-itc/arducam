/*
  Copyright (C) 2009 - 2016 National Aeronautics and Space Administration. All
...
static void CAM_Cmd_Test_NOOP(void) {
  ...
  UtAssert_True(CAM_AppData.HkTelemetryPkt.CommandErrorCount == 20,
                "cam cmd error count");
}
...
static void CAM_Cmd_Test_RESET_COUNTERS(void) {
  ...
  UtAssert_True(CAM_AppData.HkTelemetryPkt.CommandErrorCount == 0,
                "cam cmd error count");
}
...
static void CAM_Cmd_Test_HK(void) {
  /* init data */
  Ut_CFE_MSG_InitHook(&CAM_AppData.HkTelemetryPkt, CAM_HK_TLM_MID,
  ...
  CAM_Hk_tlm_t *HkTelemetryPkt =
      (CAM_Hk_tlm_t *)Ut_CFE_SB_FindPacket(CAM_HK_TLM_MID, 1);
  UtAssert_True(HkTelemetryPkt != NULL, "cam HkTelemetryPkt packet");
  if (HkTelemetryPkt) {
    UtAssert_True(HkTelemetryPkt->CommandCount == 10,
                  "cam HkTelemetryPkt cmd error count");
    UtAssert_True(HkTelemetryPkt->CommandErrorCount == 20,
                  "cam HkTelemetryPkt cmd error count");
  }
}
...
static void CAM_Cmd_Test_INVALID_CC(void) {
  ...
  /* cmd counters */
  UtAssert_True(CAM_AppData.HkTelemetryPkt.CommandCount == 10, "cam cmd count");
  UtAssert_True(CAM_AppData.HkTelemetryPkt.CommandErrorCount == 21,
                "cam cmd error count");
