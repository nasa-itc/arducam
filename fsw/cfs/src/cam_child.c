...
**
**  Purpose:
** 		   Read the camera FIFO until commanded to stop, complete, or
*error occurs.
*/
int32_t CAM_fifo(uint16 *x, uint8 *status) {
  ...
  while ((*status > 0) && (*status <= 8) &&
         (CAM_AppData.Exp_Pkt.msg_count <
          ((CAM_AppData.Exp_Pkt.length / CAM_DATA_SIZE) + 1)))
  // Status is used to track key points such as start and end of the image
  // Limiting this number ensures that cycling through the FIFO repeatedly is
  // avoided
  {
    // Read a packet
    OS_MutSemTake(CAM_AppData.data_mutex);
...
**
**  Purpose:
** 		   The experiment runs until the parent dies or is commanded to
*stop by the parent
** 		   but has the ability to be paused and resumed depending on the
*current state.
*/
...
int32_t CAM_ChildInit(void) {
  ...
  result = CFE_ES_CreateChildTask(&CAM_AppData.ChildTaskID, CAM_CHILD_TASK_NAME,
                                  CAM_ChildTask, 0, CAM_CHILD_TASK_STACK_SIZE,
                                  CAM_CHILD_TASK_PRIORITY, 0);

  if (result != CFE_SUCCESS) {
    OS_printf(
        "CAM child task initialization error: create task failed: result = %d",
        result);
  }
...
**
**  Purpose:
** 		   The child task remains active until provided the binary
*semaphore by the parent
