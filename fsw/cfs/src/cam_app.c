/* Copyright (C) 2009 - 2017 National Aeronautics and Space Administration. All
Foreign Rights are Reserved to the U.S. Government.

This software is provided "as is" without any warranty of any, kind either
express, implied, or statutory, including, but not limited to, any warranty that
the software will conform to, specifications any implied warranties of
merchantability, fitness for a particular purpose, and freedom from
infringement, and any warranty that the documentation will conform to the
program, or any warranty that the software will be error free.

In no event shall NASA be liable for any damages, including, but not limited to
direct, indirect, special or consequential damages, arising out of, resulting
from, or in any way connected with the software or its documentation.  Whether
or not based upon warranty, contract, tort or otherwise, and whether or not loss
...
void arducam_AppMain(void) {
  ...
  */
  ...
  if (status != CFE_SUCCESS) {
    CFE_EVS_SendEvent(CAM_INIT_ERR_EID, CFE_EVS_EventType_ERROR,
                      "CAM App: init error %d", status);
  ...
  */
  while (CFE_ES_RunLoop(&CAM_AppData.RunStatus) == true) {
    /*
    ** Exit performance profiling.  It will be restarted later in this while
    *loop.
    */
    ...
    */
    status = CFE_SB_ReceiveBuffer((CFE_SB_Buffer_t **)&CAM_AppData.MsgPtr,
                                  CAM_AppData.CmdPipe, CFE_SB_PEND_FOREVER);
    ...
    */
    ...
    /*
    ** If the RcvMsg() was successful, then continue to process the
    *CommandPacket()
    ...
    */
    if (status == CFE_SUCCESS) {
      ...
      */
      CFE_EVS_SendEvent(
          CAM_PIPE_ERR_EID, CFE_EVS_EventType_ERROR,
          "CAM APP: SB Pipe Read Error, CAM APP will continue with error = %d",
          status);
      // CAM_AppData.RunStatus = CFE_ES_RunStatus_APP_ERROR;
    }
  }
...
int32 CAM_AppInit(void) {
  ...
  while (true) {
    ...
    */
    status =
        CFE_SB_CreatePipe(&CAM_AppData.CmdPipe, CAM_PIPE_DEPTH, "CAM_CMD_PIPE");
    if (status != CFE_SUCCESS) {
      CFE_EVS_SendEvent(CAM_INIT_PIPE_ERR_EID, CFE_EVS_EventType_ERROR,
                        "CAM APP: Cmd pipe error %d", status);
      break;
    }

    /*
    ** Subscribe to "ground commands". Ground commands are those commands with
    *command codes
    */
    status =
        CFE_SB_Subscribe(CFE_SB_ValueToMsgId(CAM_CMD_MID), CAM_AppData.CmdPipe);
    if (status != CFE_SUCCESS) {
      CFE_EVS_SendEvent(CAM_INIT_SUB_CMD_ERR_EID, CFE_EVS_EventType_ERROR,
                        "CAM APP: Ground command subscription error %d",
                        status);
      break;
    }

    /*
    ** Subscribe to housekeeping (hk) messages.  HK messages are those messages
    *that request
    ** an app to send its HK telemetry
    */
    status = CFE_SB_Subscribe(CFE_SB_ValueToMsgId(CAM_SEND_HK_MID),
                              CAM_AppData.CmdPipe);
    if (status != CFE_SUCCESS) {
      CFE_EVS_SendEvent(CAM_INIT_SUB_HK_ERR_EID, CFE_EVS_EventType_ERROR,
                        "CAM APP: HK command subscription error %d", status);
      break;
    }
    ...
    */
    ...
    if (status != OS_SUCCESS) {
      CFE_EVS_SendEvent(CAM_MUTEX_ERR_EID, CFE_EVS_EventType_ERROR,
                        "CAM APP: Create mutex error %d", status);
      break;
    }
    ...
    */
    ...
    if (status != OS_SUCCESS) {
      CFE_EVS_SendEvent(CAM_SEMAPHORE_ERR_EID, CFE_EVS_EventType_ERROR,
                        "CAM APP: Semaphore create error %d", status);
      break;
    }
    ...
    */
    ...
    if (status != CFE_SUCCESS) {
      CFE_EVS_SendEvent(CAM_INIT_CHILD_ERR_EID, CFE_EVS_EventType_ERROR,
                        "CAM App: Child task init error %d", status);
      break;
    }
    ...
    */
    CFE_MSG_Init(CFE_MSG_PTR(CAM_AppData.HkTelemetryPkt.TlmHeader),
                 CFE_SB_ValueToMsgId(CAM_HK_TLM_MID), CAM_HK_TLM_LNGTH);

    CFE_MSG_Init(CFE_MSG_PTR(CAM_AppData.Exp_Pkt.TlmHeader),
                 CFE_SB_ValueToMsgId(CAM_EXP_TLM_MID), CAM_EXP_TLM_LNGTH);

    /*
    ** Important to send an information event that the app has initialized. this
    *is
    ...
    */
    CFE_EVS_SendEvent(CAM_STARTUP_INF_EID, CFE_EVS_EventType_INFORMATION,
                      ...
                      CAM_MAJOR_VERSION, CAM_MINOR_VERSION, CAM_REVISION,
                      CAM_MISSION_REV);
    break;
  }
...
**
**  Purpose:
**  This routine will process any packet that is received on the CAM command
*pipe.
*/
  ...
  */
  ...
  default:
    ...
    CFE_EVS_SendEvent(CAM_COMMAND_ERR_EID, CFE_EVS_EventType_ERROR,
                      "CAM App: invalid command packet, MID = 0x%x",
...
void CAM_ProcessGroundCommand(void) {
  ...
  */
  ...
  /*
  ** Ground Commands, by definition, has a command code associated with them.
  *Pull
  ** this command code from the message and then process the action associated
  *with
  ** the command code.
  */
  OS_MutSemTake(CAM_AppData.data_mutex);
  ...
  case CAM_NOOP_CC:
    ...
    */
    if (CAM_VerifyCmdLength(CAM_AppData.MsgPtr, sizeof(CAM_NoArgsCmd_t))) {
      ...
      OS_MutSemGive(CAM_AppData.data_mutex);
      CFE_EVS_SendEvent(CAM_COMMANDNOP_INF_EID, CFE_EVS_EventType_INFORMATION,
                        "CAM App: NOOP command");
    }
    break;
  ...
  */
  case CAM_EXP1_CC:
    ...
    CAM_AppData.Exp = 1;
    OS_MutSemGive(CAM_AppData.data_mutex);
    CFE_EVS_SendEvent(CAM_EXP1_EID, CFE_EVS_EventType_INFORMATION,
                      "CAM App: EXP 1 Command - Small Picture");
  ...
  */
  case CAM_EXP2_CC:
    ...
    CAM_AppData.Exp = 2;
    OS_MutSemGive(CAM_AppData.data_mutex);
    CFE_EVS_SendEvent(CAM_EXP2_EID, CFE_EVS_EventType_INFORMATION,
                      "CAM App: EXP 2 Command - Medium Picture");
    OS_BinSemGive(CAM_AppData.sem_id);
    break;
  /*
  ** EXP 3 - Large
  */
  case CAM_EXP3_CC:
    ...
    CAM_AppData.Exp = 3;
    OS_MutSemGive(CAM_AppData.data_mutex);
    CFE_EVS_SendEvent(CAM_EXP3_EID, CFE_EVS_EventType_INFORMATION,
                      "CAM App: EXP 3 Command - Large Picture");
    OS_BinSemGive(CAM_AppData.sem_id);
    break;
  ...
  */
  case CAM_HW_CHECK_CC:
    ...
    state = CAM_init_i2c();
    if (state != OS_SUCCESS) {
      CFE_EVS_SendEvent(CAM_INIT_I2C_ERR_EID, CFE_EVS_EventType_ERROR,
                        "CAM App: I2C Failure");
    } else {
      state = CAM_init_spi();
      if (state != OS_SUCCESS) {
        CFE_EVS_SendEvent(CAM_INIT_SPI_ERR_EID, CFE_EVS_EventType_ERROR,
                          "CAM App: SPI Failure");
      } else {
        CFE_EVS_SendEvent(CAM_HW_CHECK_EID, CFE_EVS_EventType_INFORMATION,
                          "CAM App: Hardware Checked Out");
      }
    }
    break;
  ...
  */
  default:
    ...
    OS_MutSemGive(CAM_AppData.data_mutex);
    CFE_EVS_SendEvent(
        CAM_COMMAND_ERR_EID, CFE_EVS_EventType_ERROR,
        "CAM App: invalid command code for packet MID = 0x%x CC = 0x%x",
...
void CAM_ResetCounters(void) {
  ...
  CAM_AppData.HkTelemetryPkt.CommandCount = 0;
  CAM_AppData.HkTelemetryPkt.CommandErrorCount = 0;
  CFE_EVS_SendEvent(CAM_COMMANDRST_INF_EID, CFE_EVS_EventType_INFORMATION,
                    "CAM App: RESET Counters Command");
  return;
}
...
bool CAM_VerifyCmdLength(CFE_MSG_Message_t *msg, uint16 ExpectedLength) {
  ...
  */
  ...
  if (ExpectedLength != ActualLength) {
    ...
    CFE_EVS_SendEvent(
        CAM_LEN_ERR_EID, CFE_EVS_EventType_ERROR,
        "Invalid msg length: ID = 0x%X CC = %d Len = %ld Expected = %d",
        CFE_SB_MsgIdToValue(MessageID), CommandCode, ActualLength,
        ExpectedLength);
