...
int32_t CAM_config(void) {
  ...
  if (result == OS_SUCCESS) { // arducam_init()
#ifdef OV2640
                              // Change register set to camera
    data[0] = 0xFF;
    data[1] = 0x01;
...
int32_t CAM_capture_prep(void) {
  ...
  if (result == OS_SUCCESS) { // Prepare for capture
#if (defined(OV5640) || defined(OV5642))
    data[0] = 0x83;
    data[1] = 0x02;
...
int32_t CAM_capture(void) {
  ...
  if (result == OS_SUCCESS) { // Wait for capture done
    data[0] = 0x41;
    data[1] = 0x00;
...
int32_t CAM_read_fifo_length(uint32_t *length) {
  ...
  if (result == OS_SUCCESS) { // Read FIFO Length
    data[0] = 0x44;
    data[1] = 0x00;
    spi_write(&CAM_SPI, data, 2);
    spi_read(&CAM_SPI, temp, 2);
...
int32_t CAM_read_prep(char *buf, uint16_t *i) {
  ...
  if (result == OS_SUCCESS) { // Read until JPEG header
    while (temp[1] != 0xFF) {
      data[0] = 0x3D;
      data[1] = 0x00;
      spi_write(&CAM_SPI, data, 2);
...
int32_t CAM_read(char *buf, uint16_t *i, uint8_t *status) {
  ...
  if (result == OS_SUCCESS) { // Read JPEG data from FIFO
    while ((((temp[1] != 0xD9) || (temp[0] != 0x00)) ||
            ((temp_last[1] != 0xFF) || (temp_last[0] != 0x00))) &&
           (*i < CAM_DATA_SIZE)) {
      temp_last[0] = temp[0];
      temp_last[1] = temp[1];
      spiw[0] = 0x3D;
