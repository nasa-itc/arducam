...
static struct sensor_reg OV2640_800x600_JPEG[] = {
    {0xff, 0x01}, {0x11, 0x01}, {0x12, 0x00}, // Bit[6:4]: Resolution selection
...
static struct sensor_reg OV5640_JPEG_QSXGA[] = {
    // 2592x1944 QSXGA
    {0x3820, 0x40}, {0x3821, 0x26}, {0x3814, 0x11}, {0x3815, 0x11},
    {0x3803, 0x00}, {0x3807, 0x9f}, {0x3808, 0x0a}, {0x3809, 0x20},
    {0x380a, 0x07}, {0x380b, 0x98}, {0x380c, 0x0b}, {0x380d, 0x1c},
    {0x380e, 0x07}, {0x380f, 0xb0}, {0x3813, 0x04}, {0x3618, 0x04},
    {0x3612, 0x4b}, {0x3708, 0x64}, {0x3709, 0x12}, {0x370c, 0x00},
    {0x3a02, 0x07}, {0x3a03, 0xb0}, {0x3a0e, 0x06}, {0x3a0d, 0x08},
    {0x3a14, 0x07}, {0x3a15, 0xb0}, {0x4001, 0x02}, {0x4004, 0x06},
    {0x3002, 0x00}, {0x3006, 0xff}, {0x3824, 0x04}, {0x5001, 0x83},
    {0x3036, 0x69}, {0x3035, 0x31}, {0x4005, 0x1A}, {0xFFFF, 0xFF},
};

static struct sensor_reg OV5640_QSXGA2QVGA[] = {
    // 320x240 QVGA
    {0x3800, 0x00}, {0x3801, 0x00}, {0x3802, 0x00}, {0x3803, 0x00},
    {0x3804, 0xA},  {0x3805, 0x3f}, {0x3806, 0x7},  {0x3807, 0x9f},
    {0x3808, 0x1},  {0x3809, 0x40}, {0x380a, 0x0},  {0x380b, 0xf0},
    {0x380c, 0xc},  {0x380d, 0x80}, {0x380e, 0x7},  {0x380f, 0xd0},
    {0x5001, 0xa3}, {0x5680, 0x0},  {0x5681, 0x0},  {0x5682, 0xA},
...
static struct sensor_reg ov5642_dvp_fmt_jpeg_qvga[] = {
    ...
    {0x4402, 0x90},
    {0x3811,
     0xf0}, // 4:3 thumbnail 				// 0x size sensor.....
...
#ifdef STF1_DEBUG
  CFE_EVS_SendEvent(HWLIB_INIT_EID, CFE_EVS_EventType_DEBUG,
                    "CAM_LibInit(): Initializing the CAM HW");
#endif
...
int32 CAM_setSize(uint8_t size) {
  ...
  case size_320x240:
    ...
    break;
  case size_1600x1200:
    result = arducam_i2c_write_regs(
        ov5642_dvp_fmt_jpeg_qvga); // TODO: Add correct register
    break;
  case size_2592x1944:
...
static int32 arducam_i2c_write_regs(struct sensor_reg reglist[]) {
  ...
  while ((next->reg != 0xFF) || (next->val != 0xFF)) {
    test[0] = next->reg;
    test[1] = next->val;
    result = i2c_master_transaction(&CAM_I2C, CAM_ADDR, &test, 2, NULL, 0,
                                    CAM_TIMEOUT);
    ...
    /*
    test[2] = 0xFF;
    result = i2c_master_transaction(&CAM_I2C, CAM_ADDR, &test, 2, &test[2], 1,
    CAM_TIMEOUT); if (test[2] != next->val)
    {
  ...
  while ((next->reg != 0xFFFF) || (next->val != 0xFF)) {
    ...
    result = i2c_master_transaction(&CAM_I2C, CAM_ADDR, &test, 3, NULL, 0,
                                    CAM_TIMEOUT);
    OS_TaskDelay(1); // Let other processes run
    if (result != OS_SUCCESS) {
      errors++;
...
int32 CAM_ChildInit(void) {
  ...
  if (result != CFE_SUCCESS) {
    CFE_EVS_SendEvent(
        CAM_CHILD_INIT_ERR_EID, CFE_EVS_EventType_ERROR,
...
int32 CAM_state(void) {
  ...
  case CAM_LOW_VOLTAGE:
    CFE_EVS_SendEvent(CAM_LOW_VOLTAGE_EID, CFE_EVS_EventType_INFORMATION,
                      "CAM child task low voltage received");
    break;

  case CAM_TIME:
    CFE_EVS_SendEvent(CAM_TIME_EID, CFE_EVS_EventType_INFORMATION,
                      "CAM child task timeout received");
    break;
...
int32 CAM_fifo(uint16 *x, uint8 *status) {
  ...
  // avoided
  {
    ...
    OS_MutSemGive(CAM_AppData.data_mutex);
    if (result != OS_SUCCESS) {
      CFE_EVS_SendEvent(CAM_READ_ERR_EID, CFE_EVS_EventType_ERROR,
                        "CAM read error");
      OS_MutSemTake(CAM_AppData.data_mutex);
    ...
    result = CAM_publish();
    if (result != OS_SUCCESS) {
      CFE_EVS_SendEvent(CAM_PUBLISH_ERR_EID, CFE_EVS_EventType_ERROR,
                        "CAM publish error");
      OS_MutSemTake(CAM_AppData.data_mutex);
      CAM_AppData.State = CAM_STOP;
      OS_MutSemGive(CAM_AppData.data_mutex);
...
int32 CAM_exp(void) {
  ...
  while (status == 1) { // Check state
    ...
    result = CAM_config();
    if (result != OS_SUCCESS) {
      CFE_EVS_SendEvent(CAM_CONFIG_ERR_EID, CFE_EVS_EventType_ERROR,
                        "CAM configure camera for upload error");
      OS_MutSemTake(CAM_AppData.data_mutex);
      CAM_AppData.State = CAM_STOP;
      OS_MutSemGive(CAM_AppData.data_mutex);
    }
    ...
    if (result != OS_SUCCESS) {
      CFE_EVS_SendEvent(CAM_JPEG_INIT_ERR_EID, CFE_EVS_EventType_ERROR,
                        "CAM jpeg init error");
      OS_MutSemTake(CAM_AppData.data_mutex);
      CAM_AppData.State = CAM_STOP;
      OS_MutSemGive(CAM_AppData.data_mutex);
    ...
    result = CAM_yuv422();
    if (result != OS_SUCCESS) {
      CFE_EVS_SendEvent(CAM_YUV422_ERR_EID, CFE_EVS_EventType_ERROR,
                        "CAM yuv422 error");
      OS_MutSemTake(CAM_AppData.data_mutex);
      CAM_AppData.State = CAM_STOP;
      OS_MutSemGive(CAM_AppData.data_mutex);
    ...
    result = CAM_jpeg();
    if (result != OS_SUCCESS) {
      CFE_EVS_SendEvent(CAM_JPEG_ERR_EID, CFE_EVS_EventType_ERROR,
                        "CAM jpeg error");
      OS_MutSemTake(CAM_AppData.data_mutex);
      CAM_AppData.State = CAM_STOP;
      OS_MutSemGive(CAM_AppData.data_mutex);
    ...
    result = CAM_setup();
    if (result != OS_SUCCESS) {
      CFE_EVS_SendEvent(CAM_SETUP_ERR_EID, CFE_EVS_EventType_ERROR,
                        "CAM setup error");
      OS_MutSemTake(CAM_AppData.data_mutex);
      CAM_AppData.State = CAM_STOP;
      OS_MutSemGive(CAM_AppData.data_mutex);
    ...
    if (result != OS_SUCCESS) {
      CFE_EVS_SendEvent(CAM_SET_SIZE_ERR_EID, CFE_EVS_EventType_ERROR,
                        "CAM upload size error");
      OS_MutSemTake(CAM_AppData.data_mutex);
      CAM_AppData.State = CAM_STOP;
      OS_MutSemGive(CAM_AppData.data_mutex);
    }
    ...
    if (result != OS_SUCCESS) {
      CFE_EVS_SendEvent(CAM_CAPTURE_PREP_ERR_EID, CFE_EVS_EventType_ERROR,
                        "CAM capture prep error");
      OS_MutSemTake(CAM_AppData.data_mutex);
      CAM_AppData.State = CAM_STOP;
      OS_MutSemGive(CAM_AppData.data_mutex);
    }
    ...
    result = CAM_capture();
    if (result != OS_SUCCESS) {
      CFE_EVS_SendEvent(CAM_CAPTURE_ERR_EID, CFE_EVS_EventType_ERROR,
                        "CAM capture error");
      OS_MutSemTake(CAM_AppData.data_mutex);
      CAM_AppData.State = CAM_STOP;
      OS_MutSemGive(CAM_AppData.data_mutex);
    ...
    if (result != OS_SUCCESS) {
      CFE_EVS_SendEvent(CAM_READ_FIFO_LEN_ERR_EID, CFE_EVS_EventType_ERROR,
                        "CAM read fifo length error");
      OS_MutSemTake(CAM_AppData.data_mutex);
      CAM_AppData.State = CAM_STOP;
      OS_MutSemGive(CAM_AppData.data_mutex);
    }
    ...
    OS_MutSemGive(CAM_AppData.data_mutex);
    if (result != OS_SUCCESS) {
      CFE_EVS_SendEvent(CAM_READ_PREP_ERR_EID, CFE_EVS_EventType_ERROR,
                        "CAM read prep error");
      OS_MutSemTake(CAM_AppData.data_mutex);
      CAM_AppData.State = CAM_STOP;
      OS_MutSemGive(CAM_AppData.data_mutex);
...
void CAM_ChildTask(void) {
  ...
  CFE_EVS_SendEvent(CAM_CHILD_INIT_EID, CFE_EVS_EventType_INFORMATION,
                    "CAM child task initialization complete");
  ...
  while (true) {
    ...
    case 3:
      ...
      break;
    default:
      CFE_EVS_SendEvent(CAM_CHILD_EXP_ERR_EID, CFE_EVS_EventType_ERROR,
                        "CAM experiment ID error");
      CAM_AppData.State = CAM_STOP;
      break;
    }
    OS_MutSemGive(CAM_AppData.data_mutex);
