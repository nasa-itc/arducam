...
for n in range(ARDUCAM_TEST_LOOP_COUNT):
    ...
    get_arducam_hk()
    check(f"ARDUCAM ARDUCAM_HK_TLM_T COMMANDCOUNT == 0")
    check(f"ARDUCAM ARDUCAM_HK_TLM_T COMMANDERRORCOUNT == 0")
...
for n in range(ARDUCAM_TEST_LOOP_COUNT):
    ...
    cmd(f"ARDUCAM CAM_NOOP_CC with CCSDS_LENGTH {n+2}") # Note +2 due to CCSDS already being +1
    get_arducam_hk()
...
for n in range(6, 6 + ARDUCAM_TEST_LOOP_COUNT): 
    ...
    cmd_cnt = tlm("ARDUCAM ARDUCAM_HK_TLM_T COMMANDCOUNT")
    cmd_err_cnt = tlm("ARDUCAM ARDUCAM_HK_TLM_T COMMANDERRORCOUNT")
    cmd(f"ARDUCAM CAM_NOOP_CC with CCSDS_FC {n+1}")
    get_arducam_hk()
    check(f"ARDUCAM ARDUCAM_HK_TLM_T COMMANDCOUNT == {cmd_cnt}")
    check(f"ARDUCAM ARDUCAM_HK_TLM_T COMMANDERRORCOUNT == {cmd_err_cnt+1}")
