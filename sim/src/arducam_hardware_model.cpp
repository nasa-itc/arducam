...
namespace Nos3
{
    REGISTER_HARDWARE_MODEL(ArducamHardwareModel,"ARDUCAM");
    ...
    void ArducamHardwareModel::command_callback(NosEngine::Common::Message msg)
    {
        ...
        std::string response = "ArducamHardwareModel::command_callback:  INVALID COMMAND! (Try STOP ARDUCAM)";
        boost::to_upper(command);
        if (command.compare("STOP ARDUCAM") == 0) 
        {
            _keep_running = false;
