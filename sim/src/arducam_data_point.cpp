...
namespace Nos3
{
    ...
    ArducamDataPoint::ArducamDataPoint(int16_t spacecraft, const boost::shared_ptr<Sim42DataPoint> dp)
    {
        sim_logger->trace("ArducamDataPoint::ArducamDataPoint:  42 Constructor executed");

        _arducam_data_is_valid = false;
        ...
        try 
        {
            for (int i = 0; i < lines.size(); i++) 
            {
                ...
                if (lines[i].compare(0, MSsize, MatchString.str()) == 0) 
                {
                    ...
                    std::string s;
                    iss >> s;
                    _arducam_data[0] = std::stod(s);
                    iss >> s;
                    _arducam_data[1] = std::stod(s);
                    iss >> s;
                    _arducam_data[2] = std::stod(s);

                    _arducam_data_is_valid = true;
                    
                    sim_logger->trace("ArducamDataPoint::ArducamDataPoint:  Parsed svb = %f %f %f", _arducam_data[0], _arducam_data[1], _arducam_data[2]);
                }
            }
        } 
        catch(const std::exception& e) 
        {
            ...
            _arducam_data[1] = 0.0;
            _arducam_data[2] = 0.0;
            sim_logger->error("ArducamDataPoint::ArducamDataPoint:  Parsing exception %s", e.what());
        }
    }

    // Mainly used for printing a representation of the data point
    std::string ArducamDataPoint::to_string(void) const
    {
        sim_logger->trace("ArducamDataPoint::to_string:  Executed");
        
        std::stringstream ss;

        ss << std::fixed << std::setfill(' ');
        ss << "Arducam Data Point:   Valid: ";
        ss << (_arducam_data_is_valid ? "Valid" : "INVALID");
